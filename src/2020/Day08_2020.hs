module Day08_2020
    ...
    day08Pt1 = (\(ProgrammeState _ (Accumulator a)) -> a) . snd . (last . runInstructions) . map (fromRight' . parse parseInstruction "") <$> readInstructions
...
-----------------------
    ...
    day08Pt2 = (\(ProgrammeState _ (Accumulator a)) -> a) . head . (\ instructions -> filter (\ (ProgrammeState n _) -> n == length instructions) $ map (\ n -> snd . last $ runInstructions'' (Just n) instructions) $ linesToChange $ loopMembers instructions) . map (fromRight' . parse parseInstruction "") <$> readInstructions
    ...
    canSwitchToEscape loopPaths (n, Jmp value) = (n + 1) `notElem` map fst loopPaths
    canSwitchToEscape loopPaths (n, Nop value) = (n + value) `notElem` map fst loopPaths
