...
data Space = Rounded | Cube | Space deriving (Eq, Show)
...
filterWithSpaceInformation ps = (ps, Height rMax, Width cMax)
    where
...
readData = filterWithSpaceInformation . convertToPositionList . map (mapMaybe toSpace) . lines <$> readFile "resource/2023/day14_test"
